...
**Static Rule Scoring vs. Weighted Feature Model Scoring:**
To understand the value of this model, consider a traditional static rule engine. A static rule might simply state: `IF technique == "Network Scan" THEN score = 50`. This is rigid. It fails to account for the blast radius of the attack.

In contrast, the **Weighted Feature Model** breaks the "Network Scan" down into its constituent threats. While the *Privilege Escalation* feature score might indeed be low (50) because a scan doesn't grant root, the model applies a massive feature score (95) to the *Lateral Movement* category. By multiplying these diverse feature scores by their fixed global weights (25% for Privilege vs. 15% for Lateral Movement), the model generates a nuanced, composite score (e.g., 65.8%). This composite score tells a much more accurate story: the attack isn't an immediate system compromise, but it is a highly dangerous precursor to cluster-wide infection, correctly landing it in the **HIGH** severity threshold rather than a generic static bucket.
